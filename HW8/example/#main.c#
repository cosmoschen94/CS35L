...
//function to divide input by threads
void* multithread (void* ptr)
{
    int* id = (int*)ptr; //cast
...
main( int argc, char **argv )
{
    ...
    for (int i = 0; i < nthreads; i++)
    {
       pthread_create(threading + i, NULL, multithread, (info+i));
       *(info+i+1)=*(info+i)+1;
    }

    for( int i=0; i < nthreads; i++)
        pthread_join(threading[i], NULL);  //wait and join threads when done
